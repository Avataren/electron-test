import { BrowserWindow as u, protocol as T, BrowserView as W, ipcMain as r, app as p } from "electron";
...
class I {
  ...
  constructor(e, t, s, i) {
    this.config = e, this.viteDevServerUrl = t, this.rendererDist = h.resolve(s), this.publicPath = i;
  }
  createWindow(e) {
    this.window = new u({
      width: this.config.window.width,
      height: this.config.window.height,
      icon: h.join(this.publicPath, "favicon.ico"),
      webPreferences: {
        preload: e,
        nodeIntegration: !1,
        contextIsolation: !0,
    ...
    return this.viteDevServerUrl ? (this.window.webContents.on("before-input-event", (t, s) => {
      ...
      console.warn("[WindowManager] Failed to register app protocol", t), this.isValid() && this.window.loadFile(h.join(this.rendererDist, "index.html"));
    }), this.window;
  }
  getWindow() {
    return this.window;
   ...
   */
  postMessageToRenderer(e, t, s, i) {
    if (!this.isValid()) return !1;
    const n = i?.fallbackToSend ?? !0;
    try {
      return s && s.length > 0 ? this.window.webContents.postMessage(e, t, s) : this.window.webContents.postMessage(e, t), !0;
    } catch (o) {
      console.warn("[WindowManager] postMessage failed", o);
    }
    if (!n)
      return !1;
    try {
      return this.window.webContents.send(e, t), !0;
    } catch (o) {
      console.error("[WindowManager] send fallback failed", o);
    }
    return !1;
  }
  getContentBounds() {
  ...
  async readRendererAsset(e) {
    ...
    let o = s.join("/");
    o || (o = "index.html");
    let a = h.resolve(this.rendererDist, o);
    if (!a.startsWith(this.rendererDist + h.sep) && a !== this.rendererDist)
      throw new Error("Attempted to access file outside renderer dist");
    try {
      return { data: await b.readFile(a), filePath: a };
    } catch (c) {
      if (!h.extname(a))
        return a = h.resolve(this.rendererDist, "index.html"), { data: await b.readFile(a), filePath: a };
      throw c;
    }
  }
  ...
  getMimeType(e) {
    switch (h.extname(e).toLowerCase()) {
      case ".html":
        return "text/html";
      case ".js":
      case ".mjs":
...
class S {
  ...
  updateDevToolsInsets() {
    ...
    const i = this.mainWindow.getBounds(), n = s.getBounds(), o = 2, a = Math.abs(n.width - i.width) <= o, c = Math.abs(n.height - i.height) <= o, l = { top: 0, right: 0, bottom: 0, left: 0 };
    if (a && n.height < i.height) {
      const d = Math.abs(n.y - i.y);
      Math.abs(
        i.y + i.height - (n.y + n.height)
      ) <= d ? l.bottom = n.height : l.top = n.height;
    } else if (c && n.width < i.width) {
      const d = Math.abs(n.x - i.x);
      Math.abs(
        i.x + i.width - (n.x + n.width)
      ) <= d ? l.right = n.width : l.left = n.width;
    } else {
      const d = t.getPreferredSize?.();
      d && (d.height && d.height < i.height ? l.bottom = d.height : d.width && d.width < i.width && (l.right = d.width));
    }
    this.devToolsInsets = {
...
class F {
  ...
  rendererSupportsSharedArrayBuffer = !0;
  constructor(e, t) {
  ...
  sendFrame(e, t, s) {
    if (this.rendererSupportsSharedArrayBuffer && typeof SharedArrayBuffer == "function")
      try {
        const n = this.writeFrameToSharedBuffer(e, t);
        if (this.windowManager.postMessageToRenderer(
          "webview-frame",
          {
            index: e,
            buffer: n,
          ...
          return s || this.pendingFrames.delete(e), !0;
        this.rendererSupportsSharedArrayBuffer && console.warn(
          "[OffscreenRenderer] renderer rejected SharedArrayBuffer payloads, falling back to ArrayBuffer frames"
        ), this.rendererSupportsSharedArrayBuffer = !1;
      } catch (n) {
  ...
  dispatchFrame(e) {
    ...
    const n = this.config.rendering.frameRate || 30, o = Math.max(0, Math.floor(1e3 / n)), a = this.lastSentAt.get(e) || 0, c = Date.now(), l = c - a;
    if (l < o) {
      const d = o - l, v = setTimeout(() => {
        this.sendTimers.delete(e), this.dispatchFrame(e);
      }, d);
...
const y = h.dirname(B(import.meta.url));
process.env.APP_ROOT = h.join(y, "..");
...
const D = process.env.VITE_DEV_SERVER_URL, z = h.join(process.env.APP_ROOT, "dist-electron"), R = h.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = D ? h.join(process.env.APP_ROOT, "public") : R;
const m = new I(
  f,
  D,
  R,
  process.env.VITE_PUBLIC || ""
), g = new S(f), C = new F(f, m), M = new P(
  f,
  g,
  C,
  m,
...
function A() {
  const w = h.join(y, "preload.js"), e = m.createWindow(w);
